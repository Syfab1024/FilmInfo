...
{
   // um bei Login-Szenarien auch eingeloggt zu bleiben
   ...
   /// <summary>
   /// Stellt einen Proxy ein, falls nötig
   /// </summary>
   /// <param name="proxyURL"></param>
   /// <param name="proxyPort"></param>
   /// <param name="proxyUID"></param>
   /// <param name="proxyPWD"></param>
                        ...
                        string proxyPWD)
   {
      if (proxyURL.Replace(" ", "") != "")
      {
         _WebProxy = new WebProxy(proxyURL, proxyPort);
         // ggf. Windows-AnmeldeInfos nutzen
         _WebProxy.Credentials = CredentialCache.DefaultNetworkCredentials;
         if (proxyUID.Replace(" ", "") != "")
         {
            _WebProxy.Credentials = new NetworkCredential(proxyUID, proxyPWD);
         }
      }
      else
         _WebProxy = null;
   ...
   protected override WebRequest GetWebRequest(Uri address)
   {
      ...
      ((HttpWebRequest)webRequest).ServicePoint.ConnectionLimit = 50;
      if (_WebProxy != null){webRequest.Proxy = _WebProxy;}
   ...
   {
      Encoding encoding = (webResponse is HttpWebResponse 
                           && !String.IsNullOrEmpty(((HttpWebResponse)webResponse).CharacterSet)) ? Encoding.GetEncoding(((HttpWebResponse)webResponse).CharacterSet) : Encoding.Default;
      
      
      if (((HttpWebResponse)webResponse).ContentEncoding == "deflate")
      {
         using (DeflateStream zip = new DeflateStream(webResponse.GetResponseStream(), CompressionMode.Decompress))
         {
            using (StreamReader streamReader = new StreamReader(zip, encoding))
               return streamReader.ReadToEnd();
         }
      }      
      else if (((HttpWebResponse)webResponse).ContentEncoding == "gzip")
      {
         using (GZipStream zip = new GZipStream(webResponse.GetResponseStream(), CompressionMode.Decompress))
         {
            using (StreamReader streamReader = new StreamReader(zip, encoding))
               return streamReader.ReadToEnd();
         }
